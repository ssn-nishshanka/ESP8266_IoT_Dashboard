...
import serial  # For communicating with ESP8266 via serial port
...
# Function to read data from serial in the background
def read_serial():
    """
    Continuously reads data from the serial port, parses it,
    appends it to the DataFrame with a timestamp, and logs to CSV.
    """

    # Setup Serial Communication inside the thread
    ser = serial.Serial('COM3', 115200, timeout=1) # Connect to COM3 at 115200 bps, wait 1s for data before timeout
    time.sleep(2)
    ser.flushInput() # Wait 2s for device to reset, then clear old serial data
    ...
    while True:
        ...
        if line:
            try:
                temp, hum, lightDetected = line.split(',')
                temp = float(temp)
                hum = float(hum)
                lightDetected = int(lightDetected)

                new_row = [
                    time.strftime("%Y-%m-%d %H:%M:%S"),
                    temp,
                    hum,
                    "Light PRESENT" if lightDetected == 1 else "No Light"
                ]
                data.loc[len(data)] = new_row

                pd.DataFrame([new_row], columns=data.columns).to_csv(
